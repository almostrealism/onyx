    ...
    func testToolsList() {
        ...
        if case .object(let obj) = response.result,
           case .array(let tools) = obj["tools"] {
            XCTAssertEqual(tools.count, 10) // show_*, clear_slot, list_slots, analyze_deps, notify, show_html, onyx_guide, onyx_status
            ...
            XCTAssertTrue(names.contains("onyx_status"))
