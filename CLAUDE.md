...
- **scp paths are NOT shell paths.** scp has spoken SFTP since OpenSSH 9.0, so the remote path goes to the SFTP server with no shell anywhere: `$HOME/x` arrives as four literal characters and fails with "No such file or directory". Use a path RELATIVE to the remote home (the SFTP session starts there) or a real absolute path; keep `$HOME` for scripts, which do run in a shell. This silently broke shared-state sync in both directions — and the failed pull looked exactly like "this host has no copy yet".
...
- **An agent says only whether an alert is `urgent` (default true); WHERE it goes is the user's setting, per machine.** `AppearanceConfig.externalDelivery` (Notification Center: every / urgent only / never) and the forwarder's own threshold (phone: urgent only / every) decide. The agent-facing `external` flag was removed — an agent can't know which machine the user is at — and is accepted and ignored if an old guide still sends it. The tool description and `onyx_guide` say, hard, that non-urgent alerts don't reach anyone and are only for "the user asked for quiet".
...
- **One version number: `Sources/OnyxVersion/OnyxVersion.swift`.** Its own dependency-free target because the app and OnyxMCP can't import each other (the bridge builds on Linux). The bundle version, the bridge's `--version` and the release tag all derive from it — `install.sh`, `package.sh` and `release.sh` read the file, and release.sh refuses to tag a version that disagrees. Bumping a release means editing that one line.
- **`OnyxVersion.bridgeProtocol` is what decides whether an installed bridge is current**, never the release number. The bridge once carried its own version and shipped calling itself 0.17 while the app was 0.16, so matching numbers prove nothing about matching binaries. `--version` prints `OnyxMCP <v> (proto <n>)`; a bridge printing no proto predates the idea and is stale by definition. Bump `bridgeProtocol` whenever the app↔bridge agreement changes.
...
**`Tests/OnyxTests/Services/RemoteScriptBudgetTests.swift` measures the ~1KB ceiling for every TTY-bound script in one place.** Add a case there for any new one, with an adversarial input — every regression of this class was a script whose size scaled with something (a path, a filter list, a probe). Scripts that travel only by pipe (`remoteScriptNoTTY`) are exempt and say so; the installer's are the only ones, and a test asserts it never adopts the TTY fallback helper.

**Prefer a pipe; keep the terminal as the fallback.** `-tt` exists solely to defeat noexec shells, and it drags in every terminal constraint: a ~1KB input queue, a line discipline that has been observed handing the remote shell a MANGLED tail (which then reports "command not found" and exits 127 with no marker), and an echo that pollutes the output. `AppState.remoteScriptNoTTY` sends the same wrapped script over a plain pipe — no limit, no echo, nothing to corrupt — and `FileBrowserManager.runScriptWithFallback` retries with `-tt` only when the completion marker doesn't come back, which is the noexec signature. New data-reading callers should use the fallback helper rather than `remoteScript` directly.
