...
/// Internal rather than private: the pool is read from the enumeration
/// and switching half of this type, which lives in
/// TerminalSessionSwitching.swift, and `private` does not reach across
/// files.
struct PoolEntry {
...
class OnyxTerminalView: NSView {
    ...
    var rapidDeaths = 0
    let maxRapidDeaths = 3
    var recoveryWaitTimer: Timer?
    var recoveryWaitStart: Date?
    ...
    /// doesn't get to spin).
    var failedGeneration: [UUID: UInt64] = [:]
    ...
    var lastEnumerationTime: Date = .distantPast
    ...
    func setSessionState(_ state: SessionConnectionState, for sessionID: String) {
        DispatchQueue.main.async {
    ...
    func clearSessionState(for sessionID: String) {
        DispatchQueue.main.async {
    ...
    func setHostState(_ state: SessionConnectionState, hostID: UUID) {
        DispatchQueue.main.async {
    ...
    @discardableResult
    func activateSession(_ session: TmuxSession, grabFocus: Bool = true) -> LocalProcessTerminalView {
    ...
    func destroyPoolEntry(_ sessionID: String) {
    ...
    func setPendingStatus(_ status: ConnectionStatus, for session: TmuxSession? = nil) {
    ...
    func clearPendingStatus(for sessionID: String) {
        DispatchQueue.main.async {
    ...
    func connectToActiveSession(grabFocus: Bool = false, isReconnect: Bool = false) {
    ...
    func probeHost(_ host: HostConfig) -> ProbeResult {
